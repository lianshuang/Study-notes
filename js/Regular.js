var _validate = {
    ...
    decideStrallz(source) {
        var regex = /^[0]*$/g;
        return regex.test(source);
    },
    decideStrallO(source) {
        var regex = /^[1]*$/g;
        return regex.test(source);
    },
     ...
     *            endindex
     */
    checkStr(source, targer, beindex, endindex) {
        var flag = false;
     ...
     ******************************************************************************/
    openWindow(url, width, height, resize) {
    ...
    openWindow(url, width, height, resize, scrollbars) {
        var mleft = (screen.width - width) / 2;
        var mtop = (screen.height - height) / 2;
        window.open(url, "", "height=" + height + ",width=" + width +
            ",location=no,menubar=no,resizable=" + resize +
     ...
     * @param {} height
     */
    showNewLessWind(url, width, height) {
        //alert(url);
    ...
    jsContains(string, substr, isIgnoreCase) {
        if (isIgnoreCase) {
     ...
     * @param {} value  参数值
     */
    urlAddParmert(url, parameter, value) {
     ...
     * @returns {number}
     */
    getIntervalWeekends(beginDate, endDate) {
        var weekends = 0;
     ...
     */
    dateDiff(strInterval, dtStart, dtEnd) { //如 'd',new Date("2016-8-22"),new Date("2016-8-25")
        switch (strInterval) {
            case 's':
